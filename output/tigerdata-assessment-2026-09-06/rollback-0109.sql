-- Separate production approval required. Stop on any definition drift.
BEGIN;
SET LOCAL lock_timeout='3s';
SET LOCAL statement_timeout='60s';
-- Retain the additive gate function. Disable dependent callers first.
DO $drift$ BEGIN
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='ops' AND p.proname='refresh_apps_page_projections_job')<>1
    OR (SELECT md5(p.prosrc) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='ops' AND p.proname='refresh_apps_page_projections_job') IS DISTINCT FROM '40142ff10dbdb5355741302c84ad28f8' THEN
    RAISE EXCEPTION 'Definition drift: ops.refresh_apps_page_projections_job; capture and review again';
  END IF;
END; $drift$;
...
DO $drift$ BEGIN
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='opportunity' AND p.proname='refresh_released_cohort_features_v2')<>1
    OR (SELECT md5(p.prosrc) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='opportunity' AND p.proname='refresh_released_cohort_features_v2') IS DISTINCT FROM '3ec4829aa09977c7d1c60d9a6310f4f6' THEN
    RAISE EXCEPTION 'Definition drift: opportunity.refresh_released_cohort_features_v2; capture and review again';
  END IF;
END; $drift$;
