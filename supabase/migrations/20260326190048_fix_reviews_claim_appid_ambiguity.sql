-- Migration: Fix appid ambiguity in claim_apps_for_reviews_sync
-- Purpose:
--   Qualify seed CTE appid references so the PL/pgSQL RETURNS TABLE
--   output variable `appid` does not conflict with bare column names.
