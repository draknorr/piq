...
The earlier Supabase-control/Tiger-evidence split is no longer the target. It would create a permanent cross-database reconciliation burden around profiles, results, evidence, cursors, and delivery. The opportunity domain will instead remain transactionally coherent in Tiger and be accessed only through an authorization-aware service. Supabase Auth user IDs are external identity keys in that domain; they do not make Supabase the store for an opportunity run or result.
...
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| Tiger opportunity domain; Supabase Auth only           | Atomic runs/results/evidence/outbox, local metric joins, one replay model, clean API boundary | Requires query-API authorization for every command/query and new workspace membership handling | **Selected target.**                                                             |
...
- Implement the selected Supabase-Auth/Tiger-opportunity boundary.
...
6. creating one transactional Tiger opportunity domain behind the query API while Supabase remains only the Auth identity and session authority;
