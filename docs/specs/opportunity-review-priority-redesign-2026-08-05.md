...
| Structural load preserved | Both corrections consume only fields already returned by the current detail/list/brief contracts.                | Shared pure helpers; no change outside admin presentation/tests/specification.                                                               | CI, merged production deployment, console review, and responsive visual verification remain.                                                              |
...
| Evidence/gaps                          | Iteration 1-9 ledgers, with measured performance, scoped-presentation control, live-run and Swords stop evidence, browser evidence, natural-run status, human labels/calibrated traction, and readiness-function evidence explicitly separated |
