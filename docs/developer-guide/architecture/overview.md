...
| Write / Control Plane | Supabase Postgres | auth, sessions, user/control data, reference data, and retained legacy exceptions |
| Semantic Layer | Cube.js | legacy and compatibility analytics reads that still have not moved to Tiger-backed contracts |
| Product Data Plane | TigerData + R2 | accepted/tested incoming ingestion and product-data writer paths, archived evidence payloads, admin product projections, and contract-serving slices |
...
- **TigerData + R2** are primary for accepted and tested incoming ingestion/product-data paths and the current Tiger-backed product projections.
- **Supabase** is retained for auth/session data, user/control state, reference data, and documented legacy exceptions.
- **TigerData (Timescale)** is the read-optimized target for contract-backed chat/search paths through `query-api` and for documented admin page projections through server-side `runTigerQuery`.
...
- chat and contract workflows should call `query-api`
- `query-api` should call TigerData for contract execution
- admin page surfaces that are explicitly Tiger-backed, such as `/apps`, `/companies`, and `/unreleased`, may read TigerData directly from server-side code through `runTigerQuery`
- browser/client code should not expose Tiger connection strings
...
| `/apps` | admin server -> Tiger projections | TigerData | uses `runTigerQuery`; falls back only when documented in code |
| `/companies` | admin server -> Tiger aggregate queries | TigerData | uses `runTigerQuery` for company analytics |
| `/unreleased` | admin server -> `metrics.unreleased_games_projection` | TigerData | upcoming-game tracker and drawer timeline |
...
Supabase
  ...
  - retained page-facing RPCs and projections
        ↓
...
TigerData + admin server reads
  - /apps, /companies, and /unreleased projections/queries
        ↓
Next.js dashboard
```
...
- unreleased games tracker at `/unreleased`
- admin pages for users, waitlist, usage, and system health
- internal APIs for chat, alerts, pins, Unreleased Games, and Change Feed
...
- legacy product read surfaces and admin/debugging data that are explicitly still retained
...
Do not add new incoming product or ingestion writes to Supabase. Accepted/tested ingestion paths write to TigerData and R2 directly, while retained legacy surfaces stay read-only or separately documented until migrated.

### TigerData Read Planes

TigerData serves two read patterns:

- contract-backed chat/search runtime through `apps/query-api` and `packages/data-plane`
- documented admin page projections through server-side `runTigerQuery` in `apps/admin`
...
Admin product reads currently include:
...
- **TigerData + R2** are primary for accepted/tested incoming ingestion and product-data paths and documented Tiger-backed product projections.
- **Supabase** remains authoritative for auth/session/control data, reference data, and retained legacy exceptions.
- **TigerData** is the current contract-serving read plane, accepted product-data target, and source for current Tiger-backed admin projections.
...
- Vercel preview and production need server-side `TIGER_PRIMARY_URL` for Tiger-backed admin page reads
