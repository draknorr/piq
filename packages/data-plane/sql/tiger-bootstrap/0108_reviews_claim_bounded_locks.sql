-- Apply only after explicit approval. Retain seed coverage, all lane quotas,
-- override precedence, due/expiry boundaries and deterministic ordering.
-- Lock only selected rows, refilling within each lane via SKIP LOCKED, then
-- reallocate unused quotas. Retain the existing eligibility scans initially;
-- a shared materialized filter was slower in the isolated fixture benchmark.
BEGIN;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '60s';

-- BEGIN GENERATED DEFINITION GUARD
DO $drift$ BEGIN
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='ops' AND p.proname='claim_apps_for_reviews_sync')<>1
    OR (SELECT md5(p.prosrc) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='ops' AND p.proname='claim_apps_for_reviews_sync') IS DISTINCT FROM 'a688d058341c67807cd49e947de89cae' THEN
    RAISE EXCEPTION 'Definition drift: ops.claim_apps_for_reviews_sync; capture and review again';
  END IF;
END; $drift$;
-- END GENERATED DEFINITION GUARD
CREATE OR REPLACE FUNCTION ops.claim_apps_for_reviews_sync(p_worker_id text, p_limit integer DEFAULT 100, p_claim_ttl_minutes integer DEFAULT 15, p_launch_limit integer DEFAULT 25, p_change_limit integer DEFAULT 20, p_active_limit integer DEFAULT 35, p_backfill_limit integer DEFAULT 19, p_unknown_limit integer DEFAULT 1)
 RETURNS TABLE(appid integer, lane text, priority_score integer, velocity_tier text, hours_overdue numeric, last_known_total_reviews integer, last_reviews_sync timestamp with time zone)
 LANGUAGE plpgsql
 SET search_path TO 'ops', 'legacy', 'metrics', 'public'
AS $function$
DECLARE
    ...
    v_lane_seed_limit integer := LEAST(GREATEST(v_requested_limit * 50, 1000), 5000);
...
BEGIN
    ...
    active_seed AS (
      ...
      WHERE COALESCE(s.is_syncable, true) = true
        ...
        AND (
          ...
          OR COALESCE(s.review_velocity_tier, 'unknown') IN ('high', 'medium')
    ...
    backfill_seed AS (
      ...
      WHERE COALESCE(s.is_syncable, true) = true
        ...
        AND (
          ...
          OR COALESCE(s.priority_score, 0) >= 50
    ...
    unknown_seed AS (
      ...
      WHERE COALESCE(s.is_syncable, true) = true
        ...
        AND COALESCE(s.review_velocity_tier, 'unknown') NOT IN ('high', 'medium')
        AND COALESCE(s.velocity_7d, 0) < 1
        AND COALESCE(s.priority_score, 0) < 50
        AND COALESCE(s.last_known_total_reviews, 0) < 1000
      ORDER BY
        COALESCE(s.reviews_priority_override_score, 0) DESC,
        CASE WHEN s.last_reviews_sync IS NULL THEN 0 ELSE 1 END ASC,
        COALESCE(s.next_reviews_sync, s.last_reviews_sync, v_now) ASC NULLS FIRST,
    ...
    seed_appids AS (
      ...
      UNION
      SELECT launch_seed.appid FROM launch_seed
      UNION
      SELECT change_seed.appid FROM change_seed
      UNION
      SELECT active_seed.appid FROM active_seed
      UNION
      SELECT backfill_seed.appid FROM backfill_seed
      UNION
      SELECT unknown_seed.appid FROM unknown_seed
    ),
    candidate_pool AS (
      SELECT
        s.appid,
    ...
    launch_claims AS MATERIALIZED (
      SELECT cp.*
      FROM candidate_pool cp
      JOIN sync_status s ON s.appid = cp.appid
      WHERE cp.lane = 'launch_critical' AND COALESCE(s.is_syncable, true) = true
        AND (s.next_reviews_sync IS NULL OR s.next_reviews_sync <= v_now)
        AND (s.reviews_claim_expires_at IS NULL OR s.reviews_claim_expires_at <= v_now)
      ORDER BY cp.sort_override_score DESC, cp.sort_never_synced ASC,
        cp.sort_due_at ASC NULLS FIRST, cp.priority_score DESC,
        cp.sort_total_reviews DESC, cp.appid ASC
      LIMIT GREATEST(v_launch_quota, 0)
      FOR UPDATE OF s SKIP LOCKED
    ),
    change_claims AS MATERIALIZED (
      SELECT cp.*
      FROM candidate_pool cp
      JOIN sync_status s ON s.appid = cp.appid
      WHERE cp.lane = 'change_critical' AND COALESCE(s.is_syncable, true) = true
        AND (s.next_reviews_sync IS NULL OR s.next_reviews_sync <= v_now)
        AND (s.reviews_claim_expires_at IS NULL OR s.reviews_claim_expires_at <= v_now)
      ORDER BY cp.sort_override_score DESC, cp.sort_never_synced ASC,
        cp.sort_due_at ASC NULLS FIRST, cp.priority_score DESC,
        cp.sort_total_reviews DESC, cp.appid ASC
      LIMIT GREATEST(v_change_quota, 0)
      FOR UPDATE OF s SKIP LOCKED
    ),
    active_claims AS MATERIALIZED (
      ...
      FROM candidate_pool cp
      JOIN sync_status s ON s.appid = cp.appid
      WHERE cp.lane = 'active_reviews' AND COALESCE(s.is_syncable, true) = true
        AND (s.next_reviews_sync IS NULL OR s.next_reviews_sync <= v_now)
        AND (s.reviews_claim_expires_at IS NULL OR s.reviews_claim_expires_at <= v_now)
      ORDER BY cp.sort_override_score DESC, cp.sort_never_synced ASC,
        cp.sort_due_at ASC NULLS FIRST, cp.priority_score DESC,
        cp.sort_total_reviews DESC, cp.appid ASC
      LIMIT GREATEST(v_active_quota, 0)
      FOR UPDATE OF s SKIP LOCKED
    ),
    backfill_claims AS MATERIALIZED (
      SELECT cp.*
      FROM candidate_pool cp
      JOIN sync_status s ON s.appid = cp.appid
      WHERE cp.lane = 'important_backfill' AND COALESCE(s.is_syncable, true) = true
        AND (s.next_reviews_sync IS NULL OR s.next_reviews_sync <= v_now)
        AND (s.reviews_claim_expires_at IS NULL OR s.reviews_claim_expires_at <= v_now)
      ORDER BY cp.sort_override_score DESC, cp.sort_never_synced ASC,
        cp.sort_due_at ASC NULLS FIRST, cp.priority_score DESC,
        cp.sort_total_reviews DESC, cp.appid ASC
      LIMIT GREATEST(v_backfill_quota, 0)
      FOR UPDATE OF s SKIP LOCKED
    ),
    unknown_claims AS MATERIALIZED (
      SELECT cp.*
      FROM candidate_pool cp
      JOIN sync_status s ON s.appid = cp.appid
      WHERE cp.lane = 'unknown_sweep' AND COALESCE(s.is_syncable, true) = true
        AND (s.next_reviews_sync IS NULL OR s.next_reviews_sync <= v_now)
        AND (s.reviews_claim_expires_at IS NULL OR s.reviews_claim_expires_at <= v_now)
      ORDER BY cp.sort_override_score DESC, cp.sort_never_synced ASC,
        cp.sort_due_at ASC NULLS FIRST, cp.priority_score DESC,
        cp.sort_total_reviews DESC, cp.appid ASC
      LIMIT GREATEST(v_unknown_quota, 0)
      FOR UPDATE OF s SKIP LOCKED
    ),
    primary_claims AS MATERIALIZED (
      SELECT * FROM launch_claims
      UNION ALL
      SELECT * FROM change_claims
      UNION ALL
      SELECT * FROM active_claims
      UNION ALL
      SELECT * FROM backfill_claims
      UNION ALL
      SELECT * FROM unknown_claims
    ),
    primary_count AS (
    ...
    reallocated_claims AS (
      SELECT r.*
      FROM candidate_pool r
      JOIN sync_status s ON s.appid = r.appid
      WHERE COALESCE(s.is_syncable, true) = true
        AND (s.next_reviews_sync IS NULL OR s.next_reviews_sync <= v_now)
        AND (s.reviews_claim_expires_at IS NULL OR s.reviews_claim_expires_at <= v_now)
        AND NOT EXISTS (
