-- Ensure high-velocity review candidates are not hidden behind older long-tail
-- due rows before lane ranking runs.

SET statement_timeout = '15min';

CREATE INDEX IF NOT EXISTS idx_ops_sync_status_reviews_active_due
  ON ops.sync_status (
    next_reviews_sync ASC NULLS FIRST,
    priority_score DESC,
    appid ASC
  )
  WHERE COALESCE(is_syncable, true) = true
    AND (
      review_velocity_tier IN ('high', 'medium')
      OR COALESCE(velocity_7d, 0) >= 1
    );

CREATE INDEX IF NOT EXISTS idx_ops_sync_status_reviews_priority_due
  ON ops.sync_status (
    next_reviews_sync ASC NULLS FIRST,
    priority_score DESC,
    appid ASC
  ...
  WHERE COALESCE(is_syncable, true) = true
    AND (
      COALESCE(priority_score, 0) >= 50
      OR COALESCE(last_known_total_reviews, 0) >= 1000
    );
...
COMMENT ON FUNCTION ops.claim_apps_for_reviews_sync IS
  'Claims due review apps with lane-specific candidate seeds so high-velocity and launch apps are ranked before old long-tail backlog can hide them.';
