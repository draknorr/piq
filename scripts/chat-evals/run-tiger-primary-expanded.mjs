...
async function main() {
  ...
  const gate = await evaluateTigerPrimaryGate(outDir);
  printTigerPrimaryGate(gate);

  console.log(`Tiger primary expanded eval report: ${reportPath}`);
...
function buildDefaultOutDir() {
  ...
  return path.join(DEFAULT_OUT_DIR_BASE, `tiger-primary-expanded-${timestamp}`);
}
...
async function runGenericRunner({ outDir, reportPath, origin }) {
  const env = {
    ...
    CHAT_EVAL_LOCAL_BYPASS_ENABLED: process.env.CHAT_EVAL_LOCAL_BYPASS_ENABLED || 'true',
    CHAT_TIGER_PRIMARY_MODE: process.env.CHAT_TIGER_PRIMARY_MODE || 'eval',
    CHAT_TIGER_SHADOW_MODE: process.env.CHAT_TIGER_SHADOW_MODE || 'eval',
  };
...
async function evaluateTigerPrimaryGate(outDir) {
  ...
  const evaluations = expectations.map((expectation) => {
    ...
    const tigerPrimary = result.tigerPrimary ?? {};
    const route = tigerPrimary.route ?? null;
    const matchedIntent = tigerPrimary.matchedIntent ?? null;

    if (expectation.kind === 'negative') {
      const ok = route === 'unmatched';
      return {
        ...expectation,
        matchedIntent,
        ok,
        reason: ok ? 'ok' : `expected_unmatched:${route ?? 'missing'}`,
        route,
      };
    }

    const ok = route === 'primary_success' && matchedIntent === expectation.expectedIntent;
    return {
      ...expectation,
      matchedIntent,
      ok,
...
function printTigerPrimaryGate(gate) {
  const passedCount = gate.evaluations.length - gate.failed.length;
  console.log(`Tiger primary gate: ${gate.passed ? 'PASS' : 'FAIL'} (${passedCount}/${gate.evaluations.length})`);
